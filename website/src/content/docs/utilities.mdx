...
Use this on an ancestor whose descendants use container queries. When the `container-type` behavior is loaded, the same values can be used with `data-unitone-layout`, for example `data-unitone-layout="-container-type:inline-size"`.
