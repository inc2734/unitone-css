import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';
import { Tabs } from 'nextra/components';
import { PreviewConainer, Badge } from '../../../components';

# Switcher

The direction of content flow switches depending on the content. If the width of the switcher block is smaller than the "threshold", it becomes one column.

## Usage

<PreviewConainer>
	<Switcher>
		<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
		<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
		<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
	</Switcher>
</PreviewConainer>

<Tabs items={['HTML', 'JSX']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher">
			<div data-unitone-layout="decorator -padding:1" style="--unitone--background-color: #3730a3; --unitone--color: #fff">Lorem ipsum</div>
			<div data-unitone-layout="decorator -padding:1" style="--unitone--background-color: #5b21b6; --unitone--color: #fff">dolor sit</div>
			<div data-unitone-layout="decorator -padding:1" style="--unitone--background-color: #6b21a8; --unitone--color: #fff">amet consectetur</div>
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher>
			<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
			<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
			<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
		</Switcher>
		```
	</Tabs.Tab>
</Tabs>

## Props

### <Badge>HTML</Badge> -gap:\{variation\} / <Badge>JSX</Badge> gap=\{variation\}

Gap.

Can be set from `-3` to `7`. For variations `2` and above, `clamp` is used to adjust the margins so that they do not become too large on small screens.
If it is `1` or more, adding `s` will shrink it to size `-1` on mobile. (e.g. `1s`)
If it is `2` or more, adding `m` will shrink it to size `1` on mobile. (e.g. `2m`)

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -gap:-2">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher gap={-2}>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher gap={-2}>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> -column-gap:\{variation\} / <Badge>JSX</Badge> columnGap=\{variation\}

Column gap.

Can be set from `-3` to `7`. For variations `2` and above, `clamp` is used to adjust the margins so that they do not become too large on small screens.
If it is `1` or more, adding `s` will shrink it to size `-1` on mobile. (e.g. `1s`)
If it is `2` or more, adding `m` will shrink it to size `1` on mobile. (e.g. `2m`)

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -column-gap:-2">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher columnGap={-2}>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher columnGap={-2}>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> -row-gap:\{variation\} / <Badge>JSX</Badge> rowGap=\{variation\}

Row gap.

Can be set from `-3` to `7`. For variations `2` and above, `clamp` is used to adjust the margins so that they do not become too large on small screens.
If it is `1` or more, adding `s` will shrink it to size `-1` on mobile. (e.g. `1s`)
If it is `2` or more, adding `m` will shrink it to size `1` on mobile. (e.g. `2m`)

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -row-gap:-2">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher rowGap={-2}>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher rowGap={-2}>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### Revert

The way child elements are displayed is reversed.

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -revert">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher revert>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> --unitone--threshold / <Badge>JSX</Badge> threshold=\{value\}

If the box size is smaller than this value, it switches to one column.

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher" style="--unitone--threshold: 500px">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher threshold="500px">
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher threshold="500px">
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> -align-items:\{value\} / <Badge>JSX</Badge> alignItems=\{value\}

Alignment of the row direction.

`start` / `center` / `end` / `stretch` / `baseline`

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -align-items:center">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher alignItems="center">
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher alignItems="center">
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1}>Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1}>Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1}>amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

#### <Badge>HTML</Badge> -stairs-up:\{value\} / <Badge>JSX</Badge> stairsUp=\{value\}

The direction of stairs. Default is `left`.

`left` / `right` / `up-down` / `down-up`

##### left

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -stairs:1 -stairs-up:left" style="--unitone--threshold: 200px">
			...
		</div>

		<div data-unitone-layout="switcher -stairs:1 -stairs-up:left -revert" style="--unitone--threshold: 200px">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher stairs={1} stairsUp="left" threshold="200px">
			...
		</Switcher>

		<Switcher stairs={1} stairsUp="left" threshold="200px" revert>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher stairs={1} stairsUp="left" threshold="200px">
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>

		<PreviewConainer>
			<Switcher stairs={1} stairsUp="left" threshold="200px" revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

##### right

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -stairs:1 -stairs-up:right" style="--unitone--threshold: 200px">
			...
		</div>

		<div data-unitone-layout="switcher -stairs:1 -stairs-up:right -revert" style="--unitone--threshold: 200px">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher stairs={1} stairsUp="right" threshold="200px">
			...
		</Switcher>

		<Switcher stairs={1} stairsUp="right" threshold="200px" revert>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher stairs={1} stairsUp="right" threshold="200px" revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>

		<PreviewConainer>
			<Switcher stairs={1} stairsUp="right" threshold="200px" revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

##### up-down

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -stairs:1 -stairs-up:up-down" style="--unitone--threshold: 200px">
			...
		</div>

		<div data-unitone-layout="switcher -stairs:1 -stairs-up:up-down -revert" style="--unitone--threshold: 200px">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher stairs={1} stairsUp="up-down" threshold="200px">
			...
		</Switcher>

		<Switcher stairs={1} stairsUp="up-down" threshold="200px" revert>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher stairs={1} stairsUp="up-down" threshold="200px">
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>

		<PreviewConainer>
			<Switcher stairs={1} stairsUp="up-down" threshold="200px" revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

##### down-up

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -stairs:1 -stairs-up:down-up" style="--unitone--threshold: 200px">
			...
		</div>

		<div data-unitone-layout="switcher -stairs:1 -stairs-up:down-up -revert" style="--unitone--threshold: 200px">
			...
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';
		import { Decorator } from '@inc2734/unitone-css/layout-primitives/decorator/react';

		<Switcher stairs={1} stairsUp="down-up" threshold="200px">
			...
		</Switcher>

		<Switcher stairs={1} stairsUp="down-up" threshold="200px" revert>
			...
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher stairs={1} stairsUp="down-up" threshold="200px">
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>

		<PreviewConainer>
			<Switcher stairs={1} stairsUp="down-up" threshold="200px" revert>
				<Decorator backgroundColor="#3730a3" color="#fff" padding={1} position="relative">Lorem ipsum</Decorator>
				<Decorator backgroundColor="#5b21b6" color="#fff" padding={1} position="relative">Lorem ipsum<br />dolor sit</Decorator>
				<Decorator backgroundColor="#6b21a8" color="#fff" padding={1} position="relative">amet consectetur</Decorator>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> -container-type:\{value\} / <Badge>JSX</Badge> containerType=\{value\}

Establishes the Switcher as a query container for its descendants.

Currently supported values are:

- `inline-size`

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -container-type:inline-size">
			<div>...</div>
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';

		<Switcher containerType="inline-size">
			<div>...</div>
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher containerType="inline-size">
				<div style={{ outline: '1px dotted #6366f1', padding: '1em' }}>
					Lorem ipsum dolor sit amet, consectetur adipisicing elit
				</div>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>

### <Badge>HTML</Badge> -responsive-context:\{value\} / <Badge>JSX</Badge> responsiveContext=\{value\}

Sets the responsive context for descendants to `container`, the only supported value. It does not affect the Switcher element itself.
Use `responsiveContext="container"` together with `containerType="inline-size"` on this element (HTML: `-responsive-context:container -container-type:inline-size`). The default is viewport-based behavior, and there is no viewport override inside a container context. The context becomes active only when both markers are on the same element. If `containerType="inline-size"` is omitted, `responsiveContext="container"` is ignored and the existing responsive context remains active. See [Responsive context utilities](/utilities#responsive-context) for examples.

### <Badge>Deprecated</Badge> <Badge>HTML</Badge> -fluid-reference:\{value\} / <Badge>JSX</Badge> fluidReference=\{value\}

Deprecated. Use `responsiveContext="container"` on an ancestor instead.

Uses a query container's inline size as the reference for fluid typography, spacing, padding, and gutters.

Currently supported values are:

- `cqw`

Place the Switcher inside an ancestor with `container-type: inline-size`.

<Tabs items={['HTML', 'JSX', 'Preview']}>
	<Tabs.Tab>
		```html
		<div data-unitone-layout="switcher -container-type:inline-size">
			<div data-unitone-layout="switcher -fluid-reference:cqw">
				...
			</div>
		</div>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		```jsx
		import { Switcher } from '@inc2734/unitone-css/layout-primitives/switcher/react';

		<Switcher containerType="inline-size">
			<Switcher fluidReference="cqw">
				...
			</Switcher>
		</Switcher>
		```
	</Tabs.Tab>
	<Tabs.Tab>
		<PreviewConainer>
			<Switcher containerType="inline-size">
				<Switcher fluidReference="cqw">
					<div style={{ outline: '1px dotted #6366f1', padding: '1em' }}>
						Lorem ipsum dolor sit amet, consectetur adipisicing elit
					</div>
				</Switcher>
			</Switcher>
		</PreviewConainer>
	</Tabs.Tab>
</Tabs>
