...
`responsiveContext="container"` on a supported primitive, or `.-responsive-context:container` on a plain element, switches descendants to container-based fluid tokens and responsive CSS only when `containerType="inline-size"` or `.-container-type:inline-size` is set on the same element. An unpaired context marker is ignored, leaving the existing responsive context active.

The default context is the viewport, and `container` is the only supported context value. The context applies only to descendants. Container contexts can be nested, with descendants using the nearest eligible query container. There is no viewport override inside a container context. See [Responsive context utilities](/utilities#responsive-context) for examples and the behavior when the container setup is missing.
...
Attribute-based CSS helpers and optional JavaScript-assisted features that enhance a layout or presentation pattern.

CSS-only behaviors expose values such as `-align-items:start` through `data-unitone-layout`. JavaScript-assisted behaviors cover features that depend on runtime measurement, initialization, or interaction support.
