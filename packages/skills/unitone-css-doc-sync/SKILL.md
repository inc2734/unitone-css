...
4. Compare the documented surface against the current source structure.
5. Update only the documentation that is actually affected by the code changes.
6. Keep descriptions concrete and based on the current source.
7. If docs that feed AI artifacts changed, regenerate derived files.
   - regenerate `llms.txt`
